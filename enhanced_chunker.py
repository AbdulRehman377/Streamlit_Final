...
class EnhancedChunker:
    ...
    def extract_chunks(self, raw_ocr: Dict, filename: str = "document") -> List[EnhancedChunk]:
        ...
        # === STEP 3: Figure chunks DISABLED (already present in page/text chunks) ===
        # Uncomment below if you need separate figure chunks
    ...
    def _extract_table_chunks(self, tables: List[Dict], filename: str, pages: List[Dict]) -> List[EnhancedChunk]:
        ...
        - No fallback heuristics - prevents values being promoted to headers in invoices/forms
        """
        chunks = []
        ...
        for table_idx, table in enumerate(tables):
            ...
            header_rows = set()  # Track which rows contain header cells
            ...
            for cell in cells:
                ...
                if row_idx < row_count and col_idx < col_count:
                    ...
                    # Only treat as header if Azure DI explicitly marks it (kind == "columnHeader")
            ...
            # REMOVED: Fallback heuristic that treated row 0 as headers
            # Reason: prevents values being promoted to headers in invoices/forms
            # Now uses only Azure DI tagged headers, or synthetic "Column N" headers
            ...
            if table_markdown and len(table_markdown) > 20:
                ...
                chunks.append(EnhancedChunk(
                    ...
                    metadata={
                        ...
                        "row_count": row_count,
                        "column_count": col_count,
                        # Only include DI-tagged headers, empty list if none
    ...
    def _table_to_markdown(self, grid: List[List], headers: List, header_rows: set, has_explicit_headers: bool, col_count: int) -> str:
        """
        Convert table grid to markdown format.
        
        - If has_explicit_headers: use DI headers, skip header_rows in body
        - If no explicit headers: use synthetic "Column 1", "Column 2", etc., render ALL rows
        """
        ...
        if has_explicit_headers:
            # Use Azure DI tagged headers
            header_row = " | ".join(h or "" for h in headers)
            lines.append(f"| {header_row} |")
            lines.append("|" + "|".join(["---"] * len(headers)) + "|")
            
            # Data rows - skip rows that were tagged as header rows by DI
            for row_idx, row in enumerate(grid):
                if row_idx in header_rows:
                    continue  # Skip header rows in body to avoid duplication
                row_content = " | ".join(cell or "" for cell in row)
                lines.append(f"| {row_content} |")
        else:
            # No DI headers - use synthetic headers, include ALL rows as data
            synthetic_headers = [f"Column {i+1}" for i in range(col_count)]
            header_row = " | ".join(synthetic_headers)
            lines.append(f"| {header_row} |")
            lines.append("|" + "|".join(["---"] * col_count) + "|")
            
            # Include ALL rows as data (no row is skipped)
            for row in grid:
                row_content = " | ".join(cell or "" for cell in row)
                lines.append(f"| {row_content} |")
    ...
    def _extract_figure_chunks(self, figures: List[Dict], content: str, filename: str) -> List[EnhancedChunk]:
        """Extract chunks for figures/images - only caption, no surrounding text."""
        chunks = []
        ...
        for fig_idx, figure in enumerate(figures):
            ...
            # Get caption ONLY - not surrounding text
            caption = ""
            
            # Method 1: Use explicit caption field if available (preferred)
            caption_obj = figure.get("caption")
            if caption_obj and isinstance(caption_obj, dict):
                caption = caption_obj.get("content", "").strip()
            
            # Method 2: If no caption, use figure ID or index as identifier
            if not caption:
                fig_id = figure.get("id", "")
                caption = f"Figure {fig_id}" if fig_id else f"Figure {fig_idx + 1}"
            
            # Create a minimal figure chunk (just caption, no surrounding text)
            # The surrounding text is already in text chunks, avoid duplication
            header = self._build_header(
                filename=filename,
